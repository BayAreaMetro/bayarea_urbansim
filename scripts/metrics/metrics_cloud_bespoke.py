...
ANALYSIS_CRS     = 26910
INTERPOLATE_TO_2023 = True    # If True, interpolate base year
...
GROWTH_METRIC_COLS = ["tothh", "totemp"]   # resunits not included in growth output files
...
def build_parcel_block20_xwalk():
    """Build a parcel_id -> block_id crosswalk using polygon-on-polygon intersection.

    Parcels that straddle block boundaries produce multiple rows (one per block).  Output 
    includes parcel_block_share, which can be used for block-level GG assignment.
    ...
    # Polygon-on-polygon intersection
    ...
    # Share of each parcel's total area that falls in each block
...
def build_block_geo_xwalk(parcel_block_xwalk):
    ...
    # Classify using half-area rule
    block_geo["tra_label"] = np.where(block_geo["tra_area"] / block_geo["intersection_area_sqm"] >= 0.5, "TRA",     "Non TRA")
...
def interpolate_to_base_year(df, id_col, metric_cols, t1=2020, t2=2025, target=2023):
    """Linearly interpolate metric columns from t1 and t2 to target year.
    ...
    rows_t2 = df[df["year"] == t2][key_cols + metric_cols].rename(
        columns={c: f"{c}_t2" for c in metric_cols}
    )
...
def calc_growth(df, geo_col):
    ...
    Base-year rows have NaN for growth/share columns.
    Horizon-year rows carry total and share-of-growth.
    """
    grouped = (
...
def main():
    global INTERPOLATE_TO_2023, BASE_YEAR

    parser = argparse.ArgumentParser(
        description="Build bespoke Cloud/BAUS urbanSim growth metrics for PBA50+."
    )
    parser.add_argument(
        "--no-interpolate", dest="interpolate_to_2023",
        action="store_false",
        help="Use 2020 as base year instead of interpolating to 2023.",
    )
    parser.set_defaults(interpolate_to_2023=INTERPOLATE_TO_2023)
    args = parser.parse_args()

    INTERPOLATE_TO_2023 = args.interpolate_to_2023
    BASE_YEAR           = 2023 if INTERPOLATE_TO_2023 else 2020
    print(f"Settings: interpolate_to_2023={INTERPOLATE_TO_2023}  base_year={BASE_YEAR}")

    # Filter config lists to exclude 2025 entries when not interpolating
    file_configs             = [c for c in FILE_CONFIGS             if INTERPOLATE_TO_2023 or c["year"] != 2025]
    baus_parcel_file_configs = [c for c in BAUS_PARCEL_FILE_CONFIGS if INTERPOLATE_TO_2023 or c["year"] != 2025]
    cloud_block_file_configs = [c for c in CLOUD_BLOCK_FILE_CONFIGS if INTERPOLATE_TO_2023 or c["year"] != 2025]
    ...
    if INTERPOLATE_TO_2023:
        combo = interpolate_to_base_year(combo, "zone_id", METRIC_COLS)
    else:
    ...
    if INTERPOLATE_TO_2023:
        combo_blocks = interpolate_to_base_year(combo_blocks, "block_id", GROWTH_METRIC_COLS)
    else:
