...
def slr_protection(
    ...
    slr_area = [
        df.inundation.isin([12, 24, 10, 20, 100]),
        (df.inundation.isin([12, 24, 10, 20, 100]) & (df[geog_name].notnull())),
    ]
    slr_protected_area = [
        df.inundation == 100,
        (df.inundation == 100) & (df[geog_name].notnull()),
    ]
    ...
    for slr, slr_protected in zip(slr_area, slr_protected_area):
        ...
        # otherwise calculate the percent protected normally
            ...
            logging.debug(
                "{} total protected SLR households".format(protected_households)
            )
    ...
    df["area_alias"] = [
        "All Households",
        "Communities of Concern" if rtp == "RTP2021" else "Equity Priority Communities",
    ]
    df["hazard"] = "SLR"
