...
census-block alternatives tables
...
table, so the fitted HLCM coefficients score block covariates directly (covariates
materialized on the block table, no `join_tbls` broadcast). `block_geoid`
...
so the choice key written back to households is a single
...
(`submarket_col='submarket_id'`, majority-TAZ roll-up). Because the block
...
The steps are registered but not yet in the annual model list, and the
...
def render_block_households_to_units(choosers, unit_alternatives, deed_restricted_only):
    ...
    its block-HLCM vacancy was rolled up from; the per-block candidate pool
...
def _block_hlcm_simulate(households, residential_units, buildings, parcels_block,
    ...
    clears at the TAZ submarket grain. Because `block_alternatives` is
    ...
    on the block table, so no `join_tbls` broadcast is passed.
    ...
    Args:
        ...
        block_alternatives: The tenure's block-indexed HLCM alternatives table
            with `num_units` supply and integer `vacant_units` vacancy.
...
def block_hlcm_owner_simulate(households, residential_units, buildings,
    ...
    Args:
        ...
        block_own_alternatives: The owner block alternatives table.
...
def block_hlcm_renter_simulate(households, residential_units, buildings,
    ...
    Args:
        ...
        buildings: The orca `buildings` table (placed-household parcel lookup).
        parcels_block: The areal parcel-to-block crosswalk table.
        block_rent_alternatives: The renter block alternatives table.
        price_settings: The `price_settings` injectable configuring equilibration.
