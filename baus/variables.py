...
# deprecated the undocumented perffoot variable - use well defined and visually similar urban area instead
@orca.column('parcels', cache=True)
def urban_footprint(parcels_urban_footprint):
    return parcels_urban_footprint.in_urban_area
...
def juris_ave_income(households, buildings, parcels_geography, parcels):
    ...
    h = orca.merge_tables("households",
                          ...
                          columns=[juris_col, "income"])
    # get median income by jurisdiction
    s = h.groupby(h[juris_col]).income.quantile(.5)
    ...
    return misc.reindex(s, parcels_geography[juris_col]).\
